...
import { borrowNToken, getAllowance, useGetUserMarketInfo } from 'clients/api';
...
describe('pages/Dashboard/BorrowRepayModal/Borrow', () => {
  ...
  it('renders without crashing', () => {
    renderComponent(<Borrow asset={fakeAsset} onClose={noop} isMiaEnabled />);
  });
  ...
  it('renders correct token borrowable amount when asset liquidity is higher than maximum amount of tokens user can borrow before reaching their borrow limit', async () => {
    ...
    const { getByText } = renderComponent(
      <Borrow asset={customFakeAsset} onClose={noop} isMiaEnabled />,
      {
        authContextValue: {
          account: {
  ...
  it('disables submit button if amount to borrow requested would make user borrow balance go higher than their borrow limit', async () => {
    const { getByText, getByTestId } = renderComponent(
      <Borrow asset={fakeAsset} onClose={noop} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('lets user borrow tokens, then displays successful transaction modal and calls onClose callback on success', async () => {
    ...
    (borrowNToken as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);

    const { getByText, getByTestId } = renderComponent(
      <Borrow asset={fakeAsset} onClose={onCloseMock} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
    ...
    await waitFor(() => expect(borrowNToken).toHaveBeenCalledTimes(1));
    expect(borrowNToken).toHaveBeenCalledWith({
