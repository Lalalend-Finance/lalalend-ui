...
import { getAllowance, repayNonBnbNToken, useGetUserMarketInfo } from 'clients/api';
...
describe('pages/Dashboard/BorrowRepayModal/Repay', () => {
  ...
  it('renders without crashing', () => {
    renderComponent(<Repay asset={fakeAsset} onClose={noop} isMiaEnabled />);
  });

  it('displays correct token borrow balance', async () => {
    const { getByText } = renderComponent(<Repay asset={fakeAsset} onClose={noop} isMiaEnabled />, {
      authContextValue: {
        account: {
  ...
  it('disables submit button if an amount entered in input is higher than token borrow balance', async () => {
    ...
    const { getByText, getByTestId } = renderComponent(
      <Repay asset={customFakeAsset} onClose={noop} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('disables submit button if an amount entered in input is higher than token wallet balance', async () => {
    const { getByText, getByTestId } = renderComponent(
      <Repay asset={fakeAsset} onClose={noop} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('lets user repay borrowed tokens, then displays successful transaction modal and calls onClose callback on success', async () => {
    ...
    (repayNonBnbNToken as jest.Mock).mockImplementationOnce(async () => fakeTransactionReceipt);

    const { getByText, getByTestId } = renderComponent(
      <Repay asset={fakeAsset} onClose={onCloseMock} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
    ...
    await waitFor(() => expect(repayNonBnbNToken).toHaveBeenCalledTimes(1));
    expect(repayNonBnbNToken).toHaveBeenCalledWith({
  ...
  it('lets user repay full loan that is not in BNB', async () => {
    ...
    const { getByText } = renderComponent(
      <Repay asset={fakeAsset} onClose={jest.fn()} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
  ...
  it('lets user repay full loan that is in BNB', async () => {
    ...
    const { getByText } = renderComponent(
      <Repay asset={fakeBnbAsset} onClose={jest.fn()} isMiaEnabled />,
      {
        authContextValue: {
          account: {
            address: fakeAccountAddress,
