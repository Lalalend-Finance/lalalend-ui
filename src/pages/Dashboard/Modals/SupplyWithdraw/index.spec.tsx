...
const fakeGetNTokenBalance = new BigNumber('111');
...
describe('pages/Dashboard/SupplyWithdrawUi', () => {
  ...
  it('renders without crashing', async () => {
    renderComponent(() => (
      <SupplyWithdraw onClose={jest.fn()} asset={asset} isMiaEnabled assets={assetData} />
    ));
  });

  it('asks the user to connect if wallet is not connected', async () => {
    const { getByText } = renderComponent(() => (
      <SupplyWithdraw onClose={jest.fn()} asset={fakeAsset} isMiaEnabled assets={[fakeAsset]} />
    ));
  ...
  describe('Withdraw form', () => {
    ...
    it('redeem is called when full amount is withdrawn', async () => {
      (getNTokenBalanceOf as jest.Mock).mockImplementationOnce(async () => ({
        balanceWei: fakeGetNTokenBalance,
      }));
      const { getByText } = renderComponent(
        () => <SupplyWithdraw onClose={jest.fn()} asset={asset} isMiaEnabled assets={assetData} />,
        {
      ...
      fireEvent.click(submitButton);
      await waitFor(() => expect(redeem).toHaveBeenCalledWith({ amountWei: fakeGetNTokenBalance }));
    });
