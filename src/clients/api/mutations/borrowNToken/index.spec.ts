...
import borrowNToken from '.';

describe('api/mutation/borrowNToken', () => {
  test('throws an error when request fails', async () => {
    ...
    } as unknown as NTokenContract<'mia'>;

    try {
      await borrowNToken({
        nTokenContract: fakeContract,
        amountWei: new BigNumber('10000000000000000'),
        fromAccountAddress: address,
      });

      throw new Error('borrowNToken should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns transaction receipt when request succeeds', async () => {
    ...
    const response = await borrowNToken({
      nTokenContract: fakeContract,
