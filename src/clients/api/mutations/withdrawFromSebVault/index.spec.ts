...
describe('api/mutation/withdrawFromSebVault', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      await withdrawFromSebVault({
        vaiVaultContract: fakeContract,
        fromAccountAddress: fakeFromAccountsAddress,
        amountWei: fakeAmountWei,
      });

      throw new Error('withdrawFromSebVault should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns receipt when request succeeds', async () => {
    ...
    const response = await withdrawFromSebVault({
      vaiVaultContract: fakeContract,
      fromAccountAddress: fakeFromAccountsAddress,
      amountWei: fakeAmountWei,
    });
