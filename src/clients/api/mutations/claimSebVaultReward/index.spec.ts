...
import claimSebVaultReward from '.';

const fakeFromAccountsAddress = '0x3d759121234cd36F8124C21aFe1c6852d2bEd848';

describe('api/mutation/claimSebVaultReward', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      await claimSebVaultReward({
        vaiVaultContract: fakeContract,
        fromAccountAddress: fakeFromAccountsAddress,
      });

      throw new Error('claimSebVaultReward should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns Receipt when request succeeds', async () => {
    ...
    const response = await claimSebVaultReward({
      vaiVaultContract: fakeContract,
      fromAccountAddress: fakeFromAccountsAddress,
    });
