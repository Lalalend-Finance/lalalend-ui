...
import getNTokenApySimulations from '.';

const fakeReserveFactorMantissa = new BigNumber(18);

describe('api/queries/getNTokenApySimulations', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      await getNTokenApySimulations({
        interestModelContract: fakeInterestModelContract,
        reserveFactorMantissa: fakeReserveFactorMantissa,
      });

      throw new Error('getNTokenBorrowBalance should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns the APY simulations in the correct format on success', async () => {
    ...
    const response = await getNTokenApySimulations({
      interestModelContract: fakeInterestModelContract,
      reserveFactorMantissa: fakeReserveFactorMantissa,
    });
