...
import getNTokenBalance from '.';

describe('api/queries/getNTokenBalance', () => {
  test('throws an error when request fails', async () => {
    ...
    try {
      await getNTokenBalance({
        vTokenContract: fakeContract,
        accountAddress: fakeAddress,
      });

      throw new Error('getNTokenBalance should have thrown an error but did not');
    } catch (error) {
  ...
  test('returns the balance on success', async () => {
    ...
    const response = await getNTokenBalance({
      vTokenContract: fakeContract,
      accountAddress: fakeAddress,
    });
